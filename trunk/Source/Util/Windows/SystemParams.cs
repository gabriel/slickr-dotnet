...
namespace Slickr.Util.Windows
{
	...
	/// <summary>
	/// Class to get at windows system parameters info API.
	/// </summary>
    public class SystemParams
    {

        [DllImport("user32.dll", CharSet = CharSet.Auto)]
        public static extern int SystemParametersInfo(SPI uAction, int uParam, IntPtr lpvParam, int fuWinIni);

        [DllImport("user32.dll", EntryPoint = "SystemParametersInfo", SetLastError = true)]
        public static extern bool SystemParametersInfoGet(SPI action, uint param, ref uint vparam, uint init);

        [DllImport("user32.dll", EntryPoint = "SystemParametersInfo", SetLastError = true)]
        public static extern bool SystemParametersInfoGetBool(SPI action, uint param, ref bool vparam, uint init);

		/// <summary>
		/// Get SPI.SPI_GETPOWEROFFACTIVE
		/// </summary>
		/// <returns></returns>
        public static bool GetPowerOffActive()
        {
            bool powerOffActive = false;
            if (SystemParametersInfoGetBool(SPI.SPI_GETPOWEROFFACTIVE, 0, ref powerOffActive, 0))
            {
                return powerOffActive;
            }
            return false;
        }

		/// <summary>
		/// Get SPI.SPI_GETPOWEROFFTIMEOUT
		/// </summary>
		/// <returns></returns>
        public static int GetPowerOffTimeout()
        {
            uint powerOffTimeout = 0;
            if (SystemParametersInfoGet(SPI.SPI_GETPOWEROFFTIMEOUT, 0, ref powerOffTimeout, 0))
            {
                return (int)powerOffTimeout;                
            }
            return -1;
        }
    }
